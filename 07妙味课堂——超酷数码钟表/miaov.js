...
function doSwitch()
{
	...
	for(i=0;i<g_aImg.length;i++)
	{
		if(g_aImg[i].now!=aNow[i])
		{
			if(g_imgHeigth>0)
			{
				...
				g_aImg[i].style.top=-(g_iMax-g_imgHeigth)/2+'px';//使变化的数字始终保持上下居中
			}
			else
			{
	...
	if(bEnd)
	{
		for(i=0;i<g_aImg.length;i++)//如果都变化完毕
		{
			g_aImg[i].now=aNow[i];
		}
...
function toDouble(iNum)//把数字转换成两位数
{
	if(iNum<10)
	{
		return '0'+iNum;
...
function getTimeArray()//把当前时间放在数组里面返回
{
	var oDate=new Date();
	var aNumber=[];
	...
	var iWeek=(oDate.getDay()+6)%7;//换算成星期
